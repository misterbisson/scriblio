...
Class File {
	...
	function decode($text) {
		if(!preg_match("/^\d{5}/", $text, $matches)) {
			$this->error[] = ('Record length "'.substr( $text, 0, 5 ).'" is not numeric');
			return(FALSE);
		}
		...
		if($reclen != strlen($text)) {
			$this->error[] = ( "Invalid record length: Leader says $reclen bytes, but it's actually ".strlen($text));
			return(FALSE);
		}
		
		if (substr($text, -1, 1) != END_OF_RECORD){
			$this->error[] = ("Invalid record terminator");
			return(FALSE);
		}
		...
		for ($n=0; $n<$nfields; $n++) {
			...
			if (!preg_match("/^[0-9A-Za-z]{3}$/", $tagno)) {
				$this->error[] = ("Invalid tag in directory: \"$tagno\"");
				return(FALSE);
			}
			if (!preg_match("/^\d{4}$/", $len)) {
				$this->error[] = ("Invalid length in directory, tag $tagno: \"$len\"");
				return(FALSE);
			}
			if (!preg_match("/^\d{5}$/", $offset)) {
				$this->error[] = ("Invalid offset in directory, tag $tagno: \"$offset\"");
				return(FALSE);
			}
			if ($offset + $len > $reclen) {
				$this->error[] = ("Directory entry runs off the end of the record tag $tagno ");
				return(FALSE);
			}
			...
			if ( substr($tagdata, -1, 1) == END_OF_FIELD ) {
				...
				$this->error[] = ("field does not end in end of field character in tag $tagno");
				return(FALSE);
			}
...
Class Record {
	...
	function append_fields($field) {
		if(strtolower(get_class($field)) == "field") {
			...
			$this->error[] = (sprintf("Given argument must be Field object, but was '%s'", get_class($field)));
			return(FALSE);
		}
	}
...
Class Field {
	...
	function data($data = "") {
		if(!$this->is_control) {
			$this->error[] = ("data() is only allowed for tags bigger or equal to 10");
			return(FALSE);
		}
		if($data) {
			$this->data = $data;
	...
	function replace_with($obj) {
		if(strtolower(get_class($obj)) == "field") {
			...
			$this->error[] = (sprintf("Argument must be Field-object, but was '%s'", get_class($obj)));
			return(FALSE);
		}
	}
